...
async def async_read_serialnr(hub: Any, address: int) -> str | None:
    ...
    try:
        ...
        res = decode_string(inverter_data[0:8])
...
class swatten_plugin(plugin_base):
    async def async_determineInverterType(self, hub: Any, configdict: dict[str, Any]) -> int:
        ...
        if seriesnumber.startswith("SiH3KSH"):
            invertertype = HYBRID | GEN | X1  # 1Phase 3kW HV ?
        elif seriesnumber.startswith("SiH4KSH"):
            invertertype = HYBRID | GEN | X1  # 1Phase 4kW HV?
        elif seriesnumber.startswith("SiH5KSH"):
            invertertype = HYBRID | GEN | X1  # 1Phase 45W HV?
        elif seriesnumber.startswith("SiH6KSH"):
            invertertype = HYBRID | GEN | X1  # 1Phase 4kW HV?
        elif seriesnumber.startswith("SiH5KTH"):
            invertertype = HYBRID | GEN | X3  # 3Phase 5kW HV?
        elif seriesnumber.startswith("SiH6KTH"):
            invertertype = HYBRID | GEN | X3  # 3Phase 6kW HV?
        elif seriesnumber.startswith("SiH8KTH"):
            invertertype = HYBRID | GEN | X3  # 3Phase 8kW HV
        elif seriesnumber.startswith("SiH10KTH"):
            invertertype = HYBRID | GEN | X3  # 3Phase 8kW HV?
